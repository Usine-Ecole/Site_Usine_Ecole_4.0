...
if (isset($_GET['token'])) {
    $reset_token = filter_var($_GET['token'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    // Vérifiez si le jeton de réinitialisation est valide
    $check_token_query = "SELECT * FROM user_form WHERE reset_token='$reset_token'";
    $check_token_result = mysqli_query($connexion, $check_token_query);

    if (mysqli_num_rows($check_token_result) == 1) {
        // L'utilisateur a un jeton de réinitialisation valide
        $_SESSION['reset_token'] = $reset_token;
    } else {
        $_SESSION['reset_password_error'] = "Jeton de réinitialisation invalide. Veuillez réessayer.";
                        ...
                        <div class="col-12 col-xl-5 col-lg-6 col-lg-7">
                            <div class="card">
                                <div class="card-body p-4">
                                    <h1 class="h2">Réinitialisation de mot de passe</h1>
                                    <hr>

                                    <?php
                                        if (isset($_SESSION['reset_password_success'])) {
                                            echo '<div class="alert__message success">' . $_SESSION['reset_password_success'] . '</div>';
                                            unset($_SESSION['reset_password_success']);
                                        } elseif (isset($_SESSION['reset_password_error'])) {
                                            echo '<div class="alert__message error">' . $_SESSION['reset_password_error'] . '</div>';
                                    ...
                                    <form action="reset_password_process.php" method="post">
                                        ...
                                        <div class="mt-5">
                                            <button type="submit" name="submit_reset_password" class="btn btn-lg btn-ht-primary btn-send pe-4 ps-4" id="btnSubmitResetPassword">
